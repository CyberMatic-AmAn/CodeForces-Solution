...
void s01ve(){
    const x7z t7b = 10;
    vector<string> m9c_vector(t7b);
    z1x(p,0,t7b){cin >> m9c_vector[p];}

    x7z total = 0;
    z1x(row,0,t7b){
        z1x(col,0,t7b){
            if(m9c_vector[row][col] == '.'){continue;}
            x7z xrow = (row < t7b - 1 - row) ? row : (t7b - 1 - row);
            x7z xcol = (col < t7b - 1 - col) ? col : (t7b - 1 - col);
            x7z score = 1 + (xrow < xcol ? xrow : xcol);
            total += score;
        }
    }
