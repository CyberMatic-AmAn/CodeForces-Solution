...
void s01ve(){
    x7z d2t; scanf("%lld", &d2t);
    while(d2t--){
        x7z l1b, b6c; scanf("%lld %lld", &l1b, &b6c);
        vector<k2m> a0m(l1b);

        z1x(i,0,l1b){
            x7z e4x; scanf("%lld", &e4x);
            a0m[i] = make_pair(e4x % b6c, i + 1);
        }

        sort(a0m.begin(), a0m.end(), c9q);

        z1x(i,0,l1b){ printf("%lld ", a0m[i].second); }
        puts("");
    }
}
